# -*- coding: utf-8 -*-
"""Download real futsal photos from Wikimedia Commons (rate-limit friendly)."""
...
SKIP_WORDS = ["diagram", "hockey", "tennis", "map"]
TARGET = 14
...
def candidates():
    queries = ["futsal court", "futsal game", "futsal hall", "futsal arena", "indoor soccer court", "futsal player"]
    seen = set()
    out = []
    for q in queries:
...
def main():
    ...
    for idx, (title, tu) in enumerate(cands, 1):
        tmp = os.path.join(OUT, f".tmp-{idx:02d}.jpg")
    ...
    # clean old files and rename sequentially
    for f in os.listdir(OUT):
        if f.startswith("futsal-"):
            os.remove(os.path.join(OUT, f))
    for i, (tmp, title) in enumerate(temps, 1):
        final = os.path.join(OUT, f"futsal-{i:02d}.jpg")
        os.rename(tmp, final)
        print(f"FINAL futsal-{i:02d}.jpg  <=  {title}")
